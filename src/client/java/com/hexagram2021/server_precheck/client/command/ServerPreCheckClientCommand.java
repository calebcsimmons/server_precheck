...
public class ServerPreCheckClientCommand {
    ...
    private static int executeList(CommandContext<FabricClientCommandSource> context) {
        ...
        context.getSource().sendFeedback(Component.literal("=== Installed Mods (" + mods.size() + ") ===").withStyle(ChatFormatting.GOLD));

        for (String mod : mods) {
            MutableComponent modComponent = Component.literal("  • " + mod).withStyle(ChatFormatting.GRAY);
    ...
    private static int executeBuildWhitelist(CommandContext<FabricClientCommandSource> context) {
        ...
        String preview;
        if (mods.size() > 5) {
            preview = "\"" + mods.get(0) + "\", \"" + mods.get(1) + "\", \"" + mods.get(2) + "\", ... (" + (mods.size() - 3) + " more)";
        } else {
