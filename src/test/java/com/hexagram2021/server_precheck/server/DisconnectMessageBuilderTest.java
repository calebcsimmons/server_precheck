 ...
 *
 * Note: Most tests for DisconnectMessageBuilder require SPCServerConfig to be initialized,
 * which depends on FabricLoader. These should be run as integration tests in a Minecraft
 * environment.
 *
 * Integration test scenarios to implement:
 *
 * Custom Message Tests:
 * - Should use custom disconnect message when set
 * - Should use custom message for not installed case
 *
 * Default Message Tests:
 * - Should build message with unauthorized mods
 * - Should build message with missing mods
 * - Should include modpack name when configured
 * - Should show contact admin message when no modpack name
 */
class DisconnectMessageBuilderTest {
    ...
    void testClassExists() {
        ...
        assertTrue(java.lang.reflect.Modifier.isFinal(DisconnectMessageBuilder.class.getModifiers()),
